...
def NPQ(max_NPQ, PsbS_protonation, Zx): #correct
    NPQ = 0.4 * max_NPQ * PsbS_protonation * Zx + 0.5 * max_NPQ * PsbS_protonation + 0.1 * max_NPQ * Zx
    return NPQ
...
def relative_VCCN1(Cl_driving_force): # empirical equation was obtained from Herdean et al. 2016 DOI: 10.1038/ncomms11654
    relative_VCCN1 =  332*(Cl_driving_force**3) + 30.8*(Cl_driving_force**2) + 3.6*Cl_driving_force
...
def include_derived_quantities(
    ...
    m.add_derived(
        name='pmf',
        fn=pmf,
        args=["Dpsi", "pH_lu", "pHstroma"]
    )

    m.add_derived(
        name='PsbS_protonation',
        fn=PsbS_protonation,
        args=["pKPsbS", "pH_lu"]
    )
    ...
    m.add_derived(
        name='NPQ',
        fn=NPQ,
        args=["max_NPQ", "PsbS_protonation", "Zx"]
    )

    m.add_derived(
        name='Keq_b6f',
        fn=Keq_b6f,
        args=["Em7_PC", "Em7_PQH2", "pH_lu", "pmf"]
    )
    ...
    m.add_derived(
        name='kb6f',
        fn=kb6f,
        args=["max_b6f", "b6f_content", "pKreg" , "pH_lu"]
    )

    m.add_derived(
        name='Keq_NDH1',
        fn=Keq_NDH1,
        args=["pHstroma", "pmf", "Em7_PQH2", "Em_Fd"]
    )

    m.add_derived(
        name='ATPsynthase_activity',
        fn=ATPsynthase_activity,
        args=["time"]
    )
    ...
    m.add_derived(
        name='Recomb_PSII',
        fn=Recomb_PSII,
        args=["dG_PSII_recomb", "kRecomb", "QA_red"]
    )
    ...
    m.add_derived(
        name='Cl_driving_force',
        fn=Cl_driving_force,
        args=["Dpsi", "Cl_lu", "Cl_st"]
    )

    m.add_derived(
        name='relative_VCCN1',
        fn=relative_VCCN1,
        args=["Cl_driving_force"]
    )
