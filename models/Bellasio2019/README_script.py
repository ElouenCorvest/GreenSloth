...
A quasi-steady-state scan of Photosynthetic Photon Flux Density (PPFD) and ambient CO2 concentration (Ca) at two different oxygen (O2) concentrations, 210 000 µbar in orange and 20 000 µbar in blue. The metabolic concentrations shown, from top to bottom, are Ribulose-1,5-bisphosphate (RuBP), Phosphoglycerate (PGA), and the relative Ribulose- 1,5-bisphosphate carboxylase-oxygenase (RuBisCO) activity. The two first mentioned were multiplied by the mesophyll volume per m2 of leaf (Vm) to come to a per area unit and the last is a product of the function that relatives RuBP concentration to concentration of RuBisCO active sites (f (RUBP)) and the RuBisCO activation state (Ract). The scan of PPFD is done at a fixed ambient CO2 concentration (Ca) of 400 µbar, while the scan of Ca is done at a fixed PPFD of $1500\ \mu mol\ m^{{-2}}\ s^{{-1}}$. The other parameters are kept at their default values. The PPFD values used are 50, 75, 100, 150, 200, 500, 1000, and 1500. For Ca are 400, 300, 200, 120, 100, 80, 60, 40, 400, 400, 400, 400, 500, 650, 850, 1050, and 1300. As steady-state cannot be always reached within this model, a quasi-steady-state is assumed to be at a maximum time of 1800 s. The experimental data could not be plotted, as there was no access to it. In the publication it is said that a scan of intercellular CO2 concentration (Ci) was done instead of Ca, however, due to many reasons, Ca is used here, even though the values of the x-axis are not identical.
