...
A quasi-steady-state scan of Photosynthetic Photon Flux Density (PPFD) and ambient CO2 concentra- tion (Ca) at two different oxygen (O2 ) concentrations, 210 000 µbar in orange and 20 000 µbar in blue. The rates shown, from top to bottom, are carbon assimilation (A), stomatal conductance of water vapor to the atmosphere (gs ), and rate of the reaction mediated by FNR (vFNR ). The scan of PPFD is done at a fixed ambient CO2 concentration (Ca) of 400 µbar, while the scan of Ca is done at a fixed PPFD of $1500\ \mu mol\ m^{{-2}}\ s^{{-1}}$ . The other parameters are kept at their default values. The PPFD values used are 50, 75, 100, 150, 200, 500, 1000, and 1500. For Ca are 400, 300, 200, 120, 100, 80, 60, 40, 400, 400, 400, 400, 500, 650, 850, 1050, and 1300. As a steady-state cannot always be reached within this model, a quasi- steady-state is assumed to be at a maximum time of 1800 s. This figure is recreated from figure 3 of the original publication of the Bellasio2019 model [18]. The experimental data could not be plotted because they were not accessible. In the publication, it is stated that a scan of intercellular CO2 concentration (Ci) was performed instead of Ca; however, for various reasons, Ca is used here, even though the x-axis values are not identical.
...
A quasi-steady-state scan of Photosynthetic Photon Flux Density (PPFD) and ambient CO2 concentration (Ca) at two different oxygen (O2 ) concentrations, 210 000 µbar in orange and 20 000 µbar in blue. The metabolic concentrations shown, from top to bottom, are Ribulose-1,5-bisphosphate (RuBP), Phosphoglycerate (PGA), and the relative Ribulose-1,5-bisphosphate carboxylase-oxygenase (RuBisCO) activity. The first two mentioned were multiplied by the mesophyll volume per m2 of leaf (Vm ) to come to a per area unit, and the last is a product of the function that relatives RuBP concentration to concentration of RuBisCO active sites (f (RUBP)) and the RuBisCO activation state (Ract ). The scan of PPFD is done at a fixed ambient CO2 concentration (Ca ) of 400 µbar, while the scan of Ca is done at a fixed PPFD of $1500\ \mu mol\ m^{{-2}}\ s^{{-1}}$ . The other parameters are kept at their default values. The PPFD values used are 50, 75, 100, 150, 200, 500, 1000, and 1500. For Ca are 400, 300, 200, 120, 100, 80, 60, 40, 400, 400, 400, 400, 500, 650, 850, 1050, and 1300. As a steady-state cannot always be reached within this model, a quasi-steady-state is assumed to be at a maximum time of 1800 s. This figure is recreated from figure 4 of the original publication of the Bellasio2019 model. The experimental data could not be plotted because they were not accessible. In the publication, it is stated that a scan of intercellular CO2 concentration (Ci) was performed instead of Ca ; however, for various reasons, Ca is used here, even though the x-axis values are not identical.
...
The simulations were done with an acclimation period of 400 s and a second period that reaches quasi-steady-state at 1800 s. Each period uses a different Photosynthetic Photon Flux Density (PPFD) to show a transition. On the left, a low-to-high light intensity transition is performed at $50\ \mu mol\ m^{{-2}}\ s^{{-1}}$ and $1500\ \mu mol\ m^{{-2}}\ s^{{-1}}$ , while on the right the reverse is performed. This switch is the zero point on the x-axis. The simulation is run using the default parameters, except ambient CO2 concentration (Ca) (= 350 µbar), maximum rate of RuBisCO carboxylation (Vmax|RuBisCO ) (= 0.18 mmol m−2 s−1 ), value of τ in the dark (τ0 ) (= −0.12), time constant for increase in gS (Ki|gS) (= 3600 s), and time constant for decrease in gS (Kd|gS) (= 1200 s). These parameters were chosen to mimic the experimental environment. The top row shows the carbon assimilation (A), the ATP synthase rate (vATPSynth), and rate of the reaction mediated by FNR (vFNR ). The top-middle row shows the stomatal conductance of water vapor to the atmosphere (gs), RuBisCO activation state (Ract), and function that relatives RuBP concentration to concentration of RuBisCO active sites (f (RUBP)). The bottom-middle row shows the inorganic phosphate (Pi), Ribulose-1,5-bisphosphate (RuBP), Phosphoglycerate (PGA), and dihydroxyacetone phosphate (DHAP). Lastly, the bottom row shows the ratio of Adenosine Triphosphate (ATP) and Nicotinamide Adenine Dinucleotide Phosphate (NADPH) to their respective totals. This figure is recreated from figure 5 of the original publication of the Bellasio2019 model. The experimental data could not be plotted because they were not accessible.
...
<img style='float: center' src='figures/{model_title.lower()}_fig6.svg' alt='Figure 6' width='600'/>

The simulations were done with an acclimation period of 400 s and a second period that reaches quasi-steady-state at 500 s. Each period uses a different ambient CO2 concentration (Ca ) to show a transition. On the left, an ambient-to-high carbon dioxide (CO2 ) concentration transition is shown, with 350 µbar and 1500 µbar, while on the right the reverse transition is shown. This switch is the zero point on the x-axis. The simulation is run using the default parameters, except Photosynthetic Photon Flux Density (PPFD) (= $1500\ \mu mol\ m^{{-2}}\ s^{{-1}}$ ), maximum rate of RuBisCO carboxylation (Vmax|RuBisCO ) (= 0.18 mmol m−2 s−1 ), value of τ in the dark (τ0 ) (= −0.12), time constant for increase in gS (Ki|gS ) (= 3600 s), and time constant for decrease in gS (Kd|gS ) (= 1200 s). These parameters were chosen to mimic the experimental environment. The top row shows the carbon assimilation (A), the ATP synthase rate (vATPSynth ), and rate of the reaction mediated by FNR (vFNR ). The top-middle row shows the stomatal conductance of water vapor to the atmosphere (gs ), RuBisCO activation state (Ract ), and function that relatives RuBP concentration to concentration of RuBisCO active sites (f (RUBP)). The bottom-middle row shows the inorganic phosphate (Pi ), Ribulose-1,5-bisphosphate (RuBP), Phosphoglycerate (PGA), and dihydroxyacetone phosphate (DHAP). Lastly, the bottom row shows the ratio of Adenosine Triphosphate (ATP) and Nicotinamide Adenine Dinucleotide Phosphate (NADPH) to their respective totals. This figure is recreated from figure 6 of the original publication of the Bellasio2019 model.
...
Response of the model to a transition from ambient oxygen (O2) concentration (210 000 µbar), until steady-state is reached, to low oxygen (O2) concentration (20 000 µbar). The simulation is run using the default parameters, except Photosynthetic Photon Flux Density (PPFD) (= $300\ \mu mol\ m^{{-2}}\ s^{{-1}}$ ) and ambient CO2 concentration (Ca) (= 200 µbar). Shown in the plot are carbon assimilation (A) (left y-axis) and efficiency of photosystem II (ΦPSII) (right y-axis). This figure is recreated from Figure 6 of the original publication of the Bellasio2019 model. The experimental data could not be plotted because there was no access to them.

This figure was recreated pretty well.
