...
This model can successfully show the theoretical PAM result on a generic photosynthetic organism without CEF. The simulation follows the typical behaviour of a PAM experiment, and the NPQ can be easily calculated. However, as no experimental data are presented, no qualitative assumption can be made about the certainty of the results. Nevertheless, this simulation shows that the model is not restricted to the steady-state assumption. Furthermore, recreating this figure using solely the model and the information from the publication has been very easy, and the graphs are identical. Additionally, just like the other figures, they provide their model code and a way to simulate each figure on their publicly available GitLab.
</details>
...
This figure presents results similar to those in the experimental data, which are referenced but not shown in the publication. The recreation of this figure using only the model and information from the publication is deemed successful; however, the small gap at a PPFD of around 550 µmol m<sup>-2</sup> s<sup>-1</sup> results from integration errors during simulation. This is a slight difference from the paper, which is explained in another figure by the oscillations of parameters that break the steady-state integrator. Although the recreation is deemed to be fine, it is worth noting that the publication figure does not accurately represent this slight gap. If it is a problem of recreation, or the authors have not included the gap, it can not be said for sure.
</details>
...
This recreation of the figure is also deemed successful, as the plots follow similar lines to those in the publication. However, in the publication, oscillations are marked, which makes simulating the steady state difficult. The recreation misses these areas; however, you can see small areas where the lines do not follow the common trend. These areas coincide with the areas of oscillation in the publication, thereby proving that the oscillations also exist in the recreation model. While successfully recreated, new simulations are advised to improve the outcome.
...
This figure illustrates the significance of CEF in the carbon fixation process of a plant. It is in qualitative agreement with experimental findings referenced, but not shown, in the publication. However, there are differences in these findings, which suggest that the model is missing several key components, particularly in the activation of the Calvin-Benson-Bassham cycle. The authors publicly identify what the model is missing to achieve experimental accuracy, and the figure recreation, which utilises only the model and the information from the publication, demonstrates that the figure recreation is successful.
