...
Simulated flourescence (red) and non-photochemical quenching trace (dashed) of a pulse-amplitude modulation (PAM) experiement of a generic photosynthetic organism without cyclic electron flow (CEF). The PAM protocol starts with a dark adapted plant with a photosynthetic photon flux density (PPFD) of 40 µmol m<sup>-2</sup> s<sup>-1</sup> for the first 4 minutes. Then a light period was simulated using a PPFD of 1000 µmol m<sup>-2</sup> s<sup>-1</sup> for 20 minutes, followed by another dark period with a PPFD of 40 µmol m<sup>-2</sup> s<sup>-1</sup> for the rest of the simulation until 2500 seconds have passed in total. During the entire simulation a short (0.8 seconds in the simulation) pulse using a PPFD of 5000 µmol m<sup>-2</sup> s<sup>-1</sup> was used every 2 minutes to induce the flourescence peaks. The values of these peaks, couldnthen be used to calculate the NPQ of the simulate of the plant during this simulation. To recreate an organism without a CEF, the simulation uses a version of the model, where the CEF reaction rate constant is set to 0.

This model can succesfully show the theoritical PAM result on a generic photosynthetic organism without CEF. The simulation follows the typical behaviour a PAM experiment has, and the NPQ could easily be calculated. However, as no experimental data is presented, no qualitativ assumption can be done on the certainty of the results. Nevertheless, this simulation shows, that the model is not restricted on the steady-state assumption. Furthermore, recreating this figure using solely the model and the information from the publication has been very easy and the graphs seem to be identical. Additionally, just like with the other figures, the others provide their model code and a way to simulate each figure on their publically available GitLab.
</details>
...
Steady-state fluxes (A) and concentration pool fraction (B) of the model depending on varying photosynthetic photon flux density (PPFD). In figure A, the PSI rate, the linear electron flow (LEF), and the cyclic electron flow (CEF), as well as the Mehler and plastoquinol terminal oxidase (PTOX) are represented. The LEF is calculated by multiplying the rate of PSII by two, while the rest can directly be accessed through the model. In figure B, the fraction of the total pool of adenosine triphosphate (ATP), reduced nicotinamide adenine dinucleotide phosphate (NADPH), reduced ferrodoxin (Fdred), reduced plastoquinone (PQred), and reduced plastocyanine (PCred), as well as the concentration of hydrogen peroxide (H<sub>2</sub>O<sub>2</sub>).

This figure shows similar results to experimental data, referenced but not shown in the publication. The recreation of this figure only using the model and infromation from the publication is deemed succesful, however the small gap at a PPFD of around 550 µmol m<sup>-2</sup> s<sup>-1</sup> results from integration errors during simulation. This is a small difference to the paper, which is explained in another figure by being occilations of, that breaks the steady-state integrator. Even though the recreation is deemed to be fine, it has to be noted that the publication figure does not represent this small gap. If it is a problem of recreation, or the authors have not included the gap can not be said for the certain.
</details>
...
High light (photosynthetic photon flux density (PPFD) of 1000 µmol m<sup>-2</sup> s<sup>-1</sup>) steady-state simulations of differing PGR5 activity. In figure A, the PSI rate, the linear electron flow (LEF), and the cyclic electron flow (CEF), as well as the 
the concentration of hydrogen peroxide (H<sub>2</sub>O<sub>2</sub>) are depicted while figure B shows the fraction of the total pool of adenosine triphosphate (ATP), reduced nicotinamide adenine dinucleotide phosphate (NADPH), reduced ferrodoxin (Fdred), reduced plastoquinone (PQred), and reduced plastocyanine (PCred). All data has been plotted against the log<sub>2</sub> change in PRG5, which is depicted in the model, by changing the the CEF reaction rate constant used in the simulation.

This recreation of the figure is also deemed succesful, as the plots follow the line of the publication. However, in the publication oscillations are marked, which make simulating to steady-state difficult. This is not depicted in the recreation, as it is not certain how these were depicted.
...
Simulation of fluctuating light in a wild-type and PGR5-Knockout mutant of the model. Figure A depicts the fraction of the total pool of reduced ferrodoxin (Fdred) and Figure B the rate of RuBisCO over the simulation time. The simulation was doen for a total of 600 seconds with 60 second intervals that change the used photosynthetic photon flux density (PPFD) between 600 µmol m<sup>-2</sup> s<sup>-1</sup> and 40 µmol m<sup>-2</sup> s<sup>-1</sup> in the light and dark pahse, repectively. To simulate a PGR5-Knockout mutant the cyclic electron flow (CEF) reaction rate constant is set to 0.

This figure shows the importanc eof CEF for the carbon fixation of a plant and is in qualitative agreement with experiemental findings referenced, not shown, in the publication. However there are differences to these findings, that show that the model is missing several key components, especially in the activation of the Calvin-Benson-Bassham cycle. the fact, that the authors publically call out what the model is missing to achieve experimental accuracy and that the figure recreation with only the model and the information of the publication shows that the figure recreation here is succesful.
...
Metabolic Control Analysis of the effect of specific parameters on fluxes (left) and concentrations (right) in low (top) and high light (bottom). The parameters (bottom of each plot) used are the total PSII pool (PS2) and PSI (PS1), and the rate constant of the cytochrome b<sub>6</sub> (b<sub>6</sub>f), the cyclic electron flow (PGR5), the mehler reaction (Mehler), RuBisCO cyrboxylase (RuBisCO), FBPase, SBPase,  monodehydroascorbate reductase (MDAR), the dehydroascorbate reductase (DHAR). The fluxes analysed are the rate of the values depicted on the vertical axis of the left side. The concentrations are decited on the vertical axis of the right side. The top heatmaps were calcualted using a photosynthetic photon flux density (PPFD) of 1000 µmol m<sup>-2</sup> s<sup>-1</sup> while the bottom with 100 µmol m<sup>-2</sup> s<sup>-1</sup>. All heatmaps were simulated by perturbing the parameters by &pm; 1% and comparing the resulting effect on the steady-state values of each analysed component.

This figure shows the control of select parameters on concentrations and fluxes of the model. With this, the hypothesis taken from the other figures could be validated to some degree. Furthermore, the recreation of this figure was flawless however it is difficult to understand which parameters and fluxes were used, as they have the same names and are not described in detail. However, the results recreated are near to perfect, therefore a succesful recreation.
...
Surface plots of steady-state simulations depending on differing PGR5 activity and photosynthetic photon flux density (PPFD). Both plots feature the range of PGR5 activity in a log<sub>2</sub>-fold change and the PPFD on the x- and y-axis, respectively. The former is simulated by changing the value of the cyclic electronf flow (CEF) rate constant. The left plot shows the linear electron flow (LEF), calculated by multiplying the rate of PSII by 2, and the right plot shows th econcentration of hydrogen peroxide (H<sub>2</sub>O<sub>2</sub>).

This figure shows that CEF activity should be regulated for maximal efficiency, because it lowers H<sub>2</sub>O<sub>2</sub> levels significantly. This figure recreation is also deemed succesful, as it follows the same surface plots, with ease of building.
